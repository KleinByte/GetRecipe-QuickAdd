...
const request = require("request-promise-native"); // Ensure request-promise-native is installed
...
function convertJsonLdToRecipeFormat(jsonLd) {
  return {
    ...
    : jsonLd.author ? jsonLd.author.name : 'Unknown author', // If it's a single object or undefined
...
// Main function to fetch recipe and display variables
async function fetchRecipeAndDisplay(url) {
  try {
    const recipe = await recipeFetcher.fetchAndParseRecipe(url);
    if (!recipe) {
      log("Unable to fetch or parse the recipe.");
      return;
    }

    const variables = mapRecipeToVariables(recipe);
    log("Recipe variables:");
...
// Command-line interface to input URL
const rl = readline.createInterface({
  input: process.stdin,
  output: process.stdout,
});

rl.question("Enter the Recipe URL: ", function (url) {
  fetchRecipeAndDisplay(url).then(() => rl.close());
});
